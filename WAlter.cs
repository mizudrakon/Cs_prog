...
namespace Weighted
{
    ...
    public class Graph
    {
        ...
        public void readGraph()
        {
            ...
            double l;
            int i = 1;
            Element[] exists = new Element[V + 1];
            ...
            while (readEdge(out v, out w, out l))
            {
                ...
                edge[i, w] = l;
                //representing only vertices that are endpoints of some edges
                if (exists[w] == null) {
                    e = new Element(w);
                    exists[w] = e;
                    G[i].Add(e);
    ...
    class MainClass
    {
        ...
        public static Tuple<int[], double[]> dijkstra(Graph G)
        {
            ...
            {
                v = G.graph.extractMin();//open element with the smallest key
                ...
                if (G.G[v.name] != null)
                {
                    foreach (Element u in G.G[v.name])
                    {
                        ...
                        {
                            //Console.WriteLine("u dist:{0:0.0} new:{1:0.0}", u.key, d);
                            u.key = d;
                            if (G.graph.ElDict.ContainsKey(u) == false) //u is closed
                            {
                                G.graph.add(u);//opening u
                            }
                            else //u was already open
                            {
                                G.graph.decrease(u, d); //decreasing u.key in the heap
                            }
                            p[u.name] = v.name; //v is parent of u (for now)
                            dist[u.name] = d; //u's distance is v.key + edge(v,u)
                        }
                    }
                }
                dist[v.name] = v.key;
